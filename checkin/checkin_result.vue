		...
		<view class="result-container">
			...
			<view class="right">
				...
				<view class="row">
					...
					<text class="checkin-result other red" v-if="status=='缺勤'">{{ status }}</text>
				</view>
				<view class="row">
...
export default {
	...
	onShow() {
		let vm = this;
		vm.ajax(vm.url.getUserCheckin, 'GET', null, (resp) => {
			let res = resp.data.result;
			vm.name = res.name;
			vm.photo = res.photo;
			vm.deptName = res.deptName;
			vm.address = res.address;
			vm.status = res.status;
			vm.risk = res.risk;
			vm.checkinTime = res.checkinTime;
			vm.attendanceTime = res.attendanceTime;
			vm.closingTime = res.closingTime;
			vm.checkinDays = res.checkinDays;
			vm.weekCheckin = res.weekCheckin;
		})
	},
	methods: {
